...
- `tests/pipeline/test_routes_pipeline.py`: 6 functional tests using sync `TestClient` with `reason.judge` mocked at the boundary (no live LLM traffic). Covers happy path against a real corpus PDF, 422 on flat / non-AcroForm PDFs, 422 on non-PDF bytes (regression: surfaced by docker smoke when README.md returned 500 because pypdf's `PdfReadError` was uncaught), 422 on unknown profile, 422 on unreachable profile (env-var pre-flight), and 413 on >20 MB uploads.
