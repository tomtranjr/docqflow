...
def _load(rel: str) -> bytes:
    """Read a fixture PDF from data/, or skip the test if it's not present.
    The data/ directory is gitignored, so these fixtures are only available
    locally; CI runs without them."""
    path = DATA / rel
    if not path.exists():
        pytest.skip(f"fixture not present on this machine: {path}")
    return path.read_bytes()


def test_filled_form_extracts_friendly_keys():
    pdf_bytes = _load("permit-3-8/permit_202604089128.pdf")
...
def test_blank_template_returns_none_for_text_fields():
    pdf_bytes = _load("permit-3-8/Form-3-8-Fillable-2020-04-07-FINAL_AxgX5Eg.pdf")
    fields = extract_form_3_8_fields(pdf_bytes)
...
def test_non_form_pdf_returns_all_none():
    pdf_bytes = _load("not-permit-3-8/MSDS Academic Plan Summer 2025.pdf")
    fields = extract_form_3_8_fields(pdf_bytes)
